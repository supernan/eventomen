...
void tools::UtilInterface::sentence_cut(std::string src, std::vector<std::string> &seps,
                                        std::vector<std::string> &res)
{
    ...
    while (true)
    {
        ...
        {
            string sep = seps[i];
            int pos = temp.find(sep);
            if (pos != string::npos)
            {
                string sent = temp.substr(0, pos);
                res.push_back(sent);
                int next = pos + sep.length();
                //if (next >= temp.length())
                //    next = temp.length()-1;
                string next_str = temp.substr(next);
                temp = next_str;
                is_find = true;
                break;
            }
...
bool tools::UtilInterface::read_text_from_local(std::string &local_path, std::vector<WeiboTopic_ICT::Weibo> &doc_list)
{
	...
	{
		LOG(FATAL) << "Text local path is wrong" << " " << local_path<<std::endl;
		return false;
	}
...
tools::WordSegThreadPool::WordSegThreadPool(const string &ict_conf, int thread_num, std::map<int, WeiboTopic_ICT::Weibo> &id_doc_map)
{
	WordSegThreadPool::_thread_num = thread_num;
	WordSegThreadPool::_doc_num = id_doc_map.size();
	if (!ICTCLAS_Init(ict_conf.c_str()))
	{
...
void* tools::_word_cut_thread(void *arg)
{
	...
	for (unsigned int i = start; i <= end; ++i)
	{
		...
		int ret = 0;
		int len = text.length();
		char *result = new char[len * 6];
		ret = ICTCLAS_ParagraphProcess(text.c_str(), len, result, CODE_TYPE_UNKNOWN, 0);
		                         ...
		                         std::map<std::string, int> &tmp_tf_map)
{
	...
	if (_doc_num <= 0)
	{
		LOG(ERROR) << "No docs to word seg" << std::endl;
		return false;
	}
	...
	{
		LOG(INFO)<<"Docs is too few less than thread_num!"<<std::endl;
		each_thread_docs = _doc_num;
		_thread_num = 1;
	}
	params[0].start = 0;
